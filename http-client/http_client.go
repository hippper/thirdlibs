...
func New() *SuperAgent {
	...
	s.Header["User-Agent"] = DEFAULT_USER_AGENT
...
// SendRawBodyData returns SuperAgent's itself for any next chain and takes content string as a parameter.
// Its duty is to put content to s.RawBodyData which later as the raw body content of the request
func (s *SuperAgent) SendRawBodyData(content string) *SuperAgent {
	s.RawBodyData = content
	s.TargetType = "raw"
	return s
}
...
func (s *SuperAgent) End(callback ...func(response Response, body string, errs []error)) (Response, string, []error) {
	...
	case POST, PUT:
		...
		} else if s.TargetType == "raw" {
			s.log("Post Data:", s.RawBodyData)
			req, err = http.NewRequest(s.Method, s.Url, strings.NewReader(s.RawBodyData))
		}
